...
/**
 * Parse the remoteResponseBody from error context and build a
 * "Delegated Logs" markdown section.  Returns empty string when
 * there is nothing useful or the data is already covered by other sections.
 */
function buildDelegatedLogsSection(error: CapturedError): string {
  const raw = error.context?.remoteResponseBody;
  if (typeof raw !== 'string' || raw.length === 0) return '';

  try {
    const parsed = JSON.parse(raw);

    // WordPress envelope shape (Riseup Asia format)
    ...
    const attrs = envelope.Attributes as Record<string, unknown> | undefined;

    // WordPress core error shape (non-envelope)
    const coreData = (envelope.data as Record<string, unknown>) ?? {};
    const coreError = coreData.error as Record<string, string> | undefined;
    ...
    // Delegated-at (which remote site)
    ...
    // Backend message from envelope errors
    const backendMsg = (errors?.BackendMessage ?? errors?.backendMessage) as string | undefined;
    if (backendMsg && !lines.some(l => l.includes(backendMsg))) {
      lines.push(`Message: ${backendMsg}`);
    }

    // PHP stack from envelope Errors.Backend array (already formatted file:line entries)
    const phpStack = (errors?.Backend ?? errors?.backend) as string[] | undefined;
    if (phpStack && phpStack.length > 0) {
      lines.push('');
      lines.push('PHP Stack Trace:');
      phpStack.forEach((frame, i) => lines.push(`  #${i} ${frame}`));
    }

    // WordPress core fatal error (non-envelope shape)
    if (!phpStack && coreError?.message) {
      lines.push('');
      lines.push(`PHP Fatal: ${coreError.message}`);
      if (coreError.file) lines.push(`  File: ${coreError.file}:${coreError.line ?? '?'}`);
    }

    // Plugin version
    const pluginVersion = envelope.PluginVersion as string | undefined;
    if (pluginVersion) lines.push(`\nRemote Plugin Version: ${pluginVersion}`);

    // Log hint
    const logHint = envelope.LogHint as string | undefined;
    if (logHint) lines.push(`Hint: ${logHint}`);

    if (lines.length === 0) return '';
    return lines.join('\n');
  } catch {
    // Not valid JSON — include raw truncated
...
export function generateCompactReport(
  ...
  // Delegated Logs (remote WordPress response)
  const delegatedLogs = buildDelegatedLogsSection(error);
  if (delegatedLogs) {
    sections.push(`\n### Delegated Logs (Remote WordPress)\n\n\`\`\`\n${delegatedLogs}\n\`\`\``);
  }
