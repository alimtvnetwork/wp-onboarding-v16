...
class RiseupPathUtils {
    ...
    private static function get_logger() {
        ...
        // return null so safe_log() falls back to error_log().
     ...
     * i.e. creation failed. This is the semantic inverse of ensure_dir().
     *
    ...
    public static function add_security_files($path) {
        ...
        if (RiseupBooleanHelpers::is_file_missing($htaccess_path)) {
            ...
            if (@file_put_contents($htaccess_path, $htaccess_content) === false) {
                ...
                self::safe_log('debug', '[PATH] Created .htaccess', array('path' => $htaccess_path));
            }
        }
        ...
        if (RiseupBooleanHelpers::is_file_missing($index_path)) {
            ...
            if (@file_put_contents($index_path, $index_content) === false) {
                ...
                self::safe_log('debug', '[PATH] Created index.php', array('path' => $index_path));
            }
        }
    ...
    public static function ensure_path($secure, ...$segments) {
        ...
        if (empty($path)) {
            self::safe_log('error', '[PATH] Empty path from segments', array(
                'segments' => $segments,
            ));
    ...
    public static function is_safe_path($path, $base_path) {
        ...
        if ($real_path === false) {
            ...
            if ($real_parent === false) {
                self::safe_log('warn', '[PATH] Neither path nor parent exists', array(
                    'path' => $path,
                    'parent' => $parent,
                ));
        ...
        if (!$is_safe) {
            self::safe_log('error', '[PATH] Path traversal attempt detected', array(
                'path' => $path,
                'resolved' => $real_path,
                'base' => $real_base,
            ));
    ...
    public static function delete_file($path) {
        ...
        if (!@unlink($path)) {
            $error = error_get_last();
            self::safe_log('error', '[PATH] Failed to delete file', array(
                'path' => $path,
                'error' => $error ? $error['message'] : 'Unknown error',
            ));
    ...
    public static function delete_dir($path) {
        ...
        if (!@rmdir($path)) {
            $error = error_get_last();
            self::safe_log('error', '[PATH] Failed to delete directory', array(
                'path' => $path,
                'error' => $error ? $error['message'] : 'Unknown error',
            ));
