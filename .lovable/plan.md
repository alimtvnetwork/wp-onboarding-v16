...
| I6 | **Remaining caller updates** | ✅ Batch 3 Done | Batch 1: ~20 files (main shell, Admin, UpdateResolver, ResponseTrait, RouteRegistrationTrait, PostManager). Batch 2: ~18 files (Plugin, Snapshot, Agent, Auth traits). **Batch 3: ~14 files** — Sync (SyncManifestTrait, SyncPushTrait), Error (ErrorLogHandlerTrait, ErrorSessionHandlerTrait), Status (StatusOpsTrait, StatusPayloadTrait), Upload (UploadInstallActivateTrait, UploadInstallExtractTrait, UploadParserTrait, UploadPipelineTrait, UploadZipTrait), FileSystem (FileSystemDirTrait, FileSystemPluginTrait), + PluginRouteRegistrationTrait (fixed stale callback strings for agent/sync/upload routes). All `handle_*` → `handleX`, `error_response` → `errorResponse`, `safe_execute` → `safeExecute`, `file_logger` → `fileLogger`, `log_upload_*` → `logUpload*`, `log_plugin_action` → `logPluginAction`, filesystem methods (`delete_directory` → `deleteDirectory`, `copy_directory` → `copyDirectory`, `find_plugin_file` → `findPluginFile`, etc.), route callback strings updated. Remaining: Database traits (`log_transaction`), Admin traits, ORM methods, AgentRemoteActionTrait. |
...
| J3 | **Create StatusType + PostStatusType enums** | ✅ Done | Created `StatusType` (2 cases + `isSuccess()`/`isFailed()`) and `PostStatusType` (3 cases + `isPublic()`, `validValues()`). All `POST_STATUS_*` callers migrated (PostManager, PostCrudTrait). `STATUS_*` callers (309 matches, 22 files) use backward-compat aliases — full caller migration deferred to J7 batch pass. |
