...
| I6 | **Remaining caller updates** | 🔄 Batch 2 Done | Batch 1: ~20 files (main shell, Admin, UpdateResolver, ResponseTrait, RouteRegistrationTrait, PostManager). **Batch 2: ~18 files** — Plugin (6 files: PluginExportTrait, PluginLifecycleDeleteTrait, PluginLifecycleEnableTrait, PluginListTrait, PluginLifecycleHelpersTrait, PluginRouteRegistrationTrait), Snapshot (9 files: SnapshotCrudCreateTrait, SnapshotCrudListTrait, SnapshotCrudRestoreTrait, SnapshotSettingsHandlerTrait, SnapshotExportHandlerTrait, SnapshotImportStreamTrait, SnapshotBackupExecTrait, SnapshotBackupOpsTrait, SnapshotRouteRegistrationTrait), Agent (2 files: AgentHandlerCrudTrait, AgentHandlerActionTrait), Auth (2 files: AuthPermissionTrait, AuthCredentialTrait). All `handle_*` → `handleX`, `error_response` → `errorResponse`, `safe_execute` → `safeExecute`, `file_logger` → `fileLogger`, `log_plugin_action` → `logPluginAction`, `log_exception` → `logException`, route callback strings updated. Remaining: ~15 files (Sync/Error/Status/Upload/FileSystem traits + AgentRemoteActionTrait `$file_logger` refs). |
